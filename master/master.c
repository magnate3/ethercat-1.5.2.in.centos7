...
/*****************************************************************************/
/* this in arm 32
unsigned long get_cpu_khz_arm_32(void)
{
    //return (READ_SYSREG(CNTFRQ_EL0) & CNTFRQ_MASK) / 1000;
    return READ_SYSREG32(CNTFRQ_EL0) / 1000;
}
*/
/*
...
{
     //https://github.com/NienfengYao/armv8-bare-metal/blob/master/aarch64.c
    //return (READ_SYSREG(CNTFRQ_EL0) & CNTFRQ_MASK) / 1000;
    return raw_read_cntfrq_el0() / 1000;
}
...
{
#ifdef EC_HAVE_CYCLES
    timeout_cycles = (cycles_t) EC_IO_TIMEOUT /* us */ * (get_cpu_khz() / 1000);
    ext_injection_timeout_cycles =
        (cycles_t) EC_SDO_INJECTION_TIMEOUT /* us */ * (get_cpu_khz()/ 1000);
#else
        ...
        )
{
    ...
    if (unlikely(master->debug_level > 1)) {
        ...
        EC_MASTER_DBG(master, 0, "%s()"
               ...
               (unsigned int) (cycles_end - cycles_start) * 1000 / get_cpu_khz());
    }
#endif
}
    ...
    list_for_each_entry_safe(datagram, next, &master->datagram_queue, queue) {
        ...
        if (master->devices[EC_DEVICE_MAIN].jiffies_poll -
            ...
            if (unlikely(master->debug_level > 0)) {
                ...
                time_us = (unsigned int)
                    (master->devices[EC_DEVICE_MAIN].cycles_poll -
                        datagram->cycles_sent) * 1000 / get_cpu_khz();
#else
                time_us = (unsigned int)
                    ((master->devices[EC_DEVICE_MAIN].jiffies_poll -
